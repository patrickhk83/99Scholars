...
class SearchForm {
	...
	public function toString() {
		...
		$searchFormDiv = new HtmlTag('div',array('id' => $this->uid.'_search_form','style' => 'display:none;'));
		$searchForm->addChildren($searchFormDiv);
		...
		if (empty($this->hide) || !in_array('search', $this->hide)){
			...
			foreach ( $this->searchs as $field ) {
				...
				if ($i % 2 == 1) {
					$searchFormDiv->addChildren ( $rowfluid );
					$rowfluid = new HtmlTag ( 'div', array (
							'class' => 'row-fluid show-grid'
					) );
				}
		...
		if ((empty($this->hide) || !in_array('search', $this->hide)) && !empty($this->searchs)){
			...
			$buttonGroup->addChildren ( new HtmlTag ( 'a', array (
					'class' => 'btn',
					'id' => $this->uid.'_search_button',
					'onclick' => $searchOnclick
			), ' <i class="icon-search"></i> '.btn_search ) );
			
			$buttonGroup->addChildren ( new HtmlTag ( 'a', array (
					'class' => 'btn',
					'id' => $this->uid.'_search_show',
					'onclick' => '$("#'.$this->uid.'_search_form").show();$("#'.$this->uid.'_search_hide").show();$(this).hide();',
			), ' Show' ) );
			
			$buttonGroup->addChildren ( new HtmlTag ( 'a', array (
					'class' => 'btn',
					'id' => $this->uid.'_search_hide',
					'style' => 'display:none;',
					'onclick' => '$("#'.$this->uid.'_search_form").hide();$("#'.$this->uid.'_search_show").show();$(this).hide();',
			), ' Hide' ) );
